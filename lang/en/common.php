<?php

return [
    'slogan'              => 'Welcome',
    'newsletter'          => 'Newsletter',
    'published_at'        => 'Published',
    'draft'               => 'draft',
    'active'              => 'active',
    'archived'            => 'archived',
    'reset_password_page' => 'Reset Password Page',

    'legal_documents' => [
        'terms'   => 'Terms and Conditions',
        'privacy' => 'Privacy Policy',
        'cookies' => 'Cookies Policy',
        'refund'  => 'Refund Policy',
    ],

    'legal_information' => 'Legal Information',

    'faq_page' => [
        'faq_message_title'  => 'Hello, what can we help you find?',
        'no_answer_text'     => 'Can’t find an answer? Send us a message and we’ll get back to you.',
        'search_placeholder' => 'Search for answers',
        'base_list_title'    => 'Frequently Asked Questions',
    ],

    'search' => [
        'button'     => 'Search',
        'title'      => 'Search Results',
        'no_results' => 'No results found for your search.',
    ],

    'feedback' => [
        'title'    => 'Contact us',
        'subtitle' => "Didn't find an answer? Send us a message and we will get back to you.",
        'name'     => 'Your name',
        'email'    => 'Email address',
        'message'  => 'Message',
        'submit'   => 'Send',
        'success'  => 'Thank you! Your message has been sent.'
    ],

    'loading_translations' => 'Translating...',
];